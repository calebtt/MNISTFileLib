...
// The code to test the old NN project code.
inline void TestOldFramework()
{
	/*
	...
	const auto buildRandomVector = [](size_t len)
	{
	...
	const auto imageData = readImageData();
	...
	const auto randomVector = buildRandomVector(imageData.ImageBuffer.size());
	const auto randomNeuronVector = buildRandomVector(NumberOfHiddenNeurons);
	for (size_t i = 0; i < imageData.ImageBuffer.size(); i++)
	{
		InputNode n(i, imageData.ImageBuffer[i]);
		cout << "Weight:" << n.m_weight << " ";
		inputLayer.emplace_back(n);
	}
	for (size_t i = 0; i < NumberOfHiddenNeurons; i++)
	{
		HiddenNeuron n(i, randomNeuronVector[i]);
		//cout << "Bias:" << n.m_bias << " ";
	...
	const double TValue = getRandomDouble(0.0, 15.0);
